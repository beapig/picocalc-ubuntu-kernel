...
volatile bool led_update_pending = false;  /* deferred LED apply flag */
...
void receiveEvent(int howMany) {
  ...
  switch (reg) {
    ...
    case REG_ID_CFG: {
      if (is_write) {
        reg_set_value(REG_ID_CFG, rcv_data[1]);
      }
      write_buffer[0] = reg;
      write_buffer[1] = reg_get_value(REG_ID_CFG);
    } break;
    case REG_ID_OFF:{
      if(is_write){
    ...
    case REG_ID_LED:{
      if(is_write){
        reg_set_value(REG_ID_LED, rcv_data[1]);
        led_update_pending = true;  /* defer led_apply() to main loop */
      }
      write_buffer[0] = reg;
      write_buffer[1] = reg_get_value(REG_ID_LED);
    }break;
    case REG_ID_RST:{
      if(is_write){
...
void check_pmu_int() {
  ...
  if(pmu_online > 1) {
    /* Full power-down: same behavior as long-press power key */
    analogWrite(PA8, 0);         /* LCD backlight off */
    analogWrite(PC8, 0);         /* keyboard backlight off */
    digitalWrite(PA13, LOW);     /* cut Linux SoC power */
    digitalWrite(PA14, LOW);     /* PA_EN off */
    digitalWrite(PC13, HIGH);    /* green LED off (active-low) */
    PMU.setChargingLedMode(XPOWERS_CHG_LED_CTRL_CHG);  /* orange: charge auto */
  ...
  if (pmu_flag) {
    ...
    if (PMU.isPekeyNegativeIrq()) {
      ...
      key_cb(KEY_POWER, KEY_STATE_RELEASED);
    }

...
** New Mapping Design by @beapig

!!! accroding to the keyboard firmware source code,shift + capslk/backspace/left/right/ = no output
!!! mouse_wheel scrolling can be active by R_mousebutton(as well as ']') + arrowkey,this feature is providing by
x windows system, so no need to handle in the driver.
...
|------------|----------------|------------|---------------------|------------------------|
| Up-Down    | Mouse_Up-Down  | Up-Down    | PageUp-PageDown     | Shift + Wheel_Up-Down  |
...
static uint32_t sysfs_gid_setting = 0; 
// GID of files in /sys/firmware/picocalc
...
// keymap to handle special 2ndkey combinations
...
typedef struct {
	...
	uint16_t keycode1;
    uint16_t keycode2;
    bool keycode2_pressed; // flag to track kc2 pressed state
} FnMap;

static FnMap fn_keycodes[] = {
    {0x81, 0x86, KEY_F1, KEY_F6,false}, // F1-F6
	{0x82, 0x87, KEY_F2, KEY_F7,false}, // F2-F7
	{0x83, 0x88, KEY_F3, KEY_F8,false}, // F3-F8
	{0x84, 0x89, KEY_F4, KEY_F9,false}, // F4-F9
	{0x85, 0x90, KEY_F5, KEY_F10,false}, // F5-F10

	{0xB1, 0xD0, KEY_ESC, KEY_PAUSE,false}, // Esc-Break
	{0x09, 0xD2, KEY_TAB, KEY_HOME,false}, // Tab-Home
	{0xD4, 0xD5, KEY_DELETE, KEY_END,false}, // Del-End
//	{0x0D, 0xD1, KEY_ENTER, KEY_INSERT,false}, // Enter-Insert

	{0xb5, 0xd6, KEY_UP, KEY_PAGEUP,false}, // Up-PageUp
	{0xb6, 0xd7, KEY_DOWN, KEY_PAGEDOWN,false}, // Down-PageDown
};

#define FN_MAP_SIZE (sizeof(fn_keycodes) / sizeof(fn_keycodes[0]))
...
struct kbd_ctx
{
    ...
    // flags for mouse mode
	bool mouse_mode;
    uint8_t mouse_move_dir;

	uint8_t mouse_tick;       /* incremented each work run */
    uint8_t mouse_report_div;
	
	// flags to track shift status(physical status)
	bool phy_SHR;
	bool phy_SHL;
	bool virtual_INS;
};
...
static inline int kbd_read_i2c_u8(struct i2c_client* i2c_client, uint8_t reg_addr,uint8_t* dst)
{
	int reg_value;
...
static inline int kbd_write_i2c_u8(struct i2c_client* i2c_client, uint8_t reg_addr,uint8_t src)
{
	int rc;
...
static inline int kbd_read_i2c_2u8(struct i2c_client* i2c_client, uint8_t reg_addr,uint8_t* dst)
{
	int word_value;
...
// deal with fn keys
static bool proccess_fn_key(struct kbd_ctx* ctx, struct key_fifo_item const* ev)
{
	int i;
	bool is_press = (ev->state == KEY_STATE_PRESSED);
    bool is_release = (ev->state == KEY_STATE_RELEASED);
	if (!is_press && !is_release) return false;
	for (i = 0; i < FN_MAP_SIZE; i++) {
		FnMap *row = &fn_keycodes[i];
		// check if scancode matches scancode1 or scancode2
		if (ev->scancode == row->scancode1 || ev->scancode == row->scancode2){
			if(is_press){
				if(ctx->phy_SHR){
					// R_shift is pressed, release R_shift,send keycode2,
					input_report_key(ctx->input_dev, KEY_RIGHTSHIFT, 0);
					input_report_key(ctx->input_dev, row->keycode2, 1);
					row->keycode2_pressed = true;
				} else {
					// R_shift is not pressed, send keycode1
					input_report_key(ctx->input_dev, row->keycode1, 1);
				}
			}
			else if(is_release){
				if(row->keycode2_pressed){
					// keycode2 was pressed, release it
					input_report_key(ctx->input_dev, row->keycode2, 0);
					row->keycode2_pressed = false;
				} else {
					// keycode1 was pressed, release it
					input_report_key(ctx->input_dev, row->keycode1, 0);
				}
			}
			return true;
		}
...
static void key_report_event(struct kbd_ctx* ctx,struct key_fifo_item const* ev)
{
	uint8_t keycode;

	// Only handle key pressed, held, or released events
	if ((ev->state != KEY_STATE_PRESSED) && (ev->state != KEY_STATE_RELEASED) && (ev->state != KEY_STATE_HOLD)) {
		return;
	}

	// Always update shift status
		
	if (ev->scancode == 0xA3) { // R_shift
		ctx->phy_SHR = (ev->state == KEY_STATE_PRESSED || ev->state == KEY_STATE_HOLD);
	}
	if (ev->scancode == 0xA2) { // L_shift
		ctx->phy_SHL = (ev->state == KEY_STATE_PRESSED || ev->state == KEY_STATE_HOLD);
	}
	
	// Mouse mode logic start here
	if (ctx->mouse_mode){
		switch(ev->scancode){
			/* KEY_RIGHT */		
			case 0xb7:
				...
				{
					input_report_rel(ctx->input_dev, REL_X, 3);
			...
			case 0xb6:				
				...
				{
					input_report_rel(ctx->input_dev, REL_Y, 3);
			...
			case 0xb5:
				...
				else if (ev->state == KEY_STATE_PRESSED)
				{
					...
					// input_report_rel(ctx->input_dev, REL_Y, -10);
	...
	// !!! dule to the keyboard firmware shift = caps and backspace will not output patch for it
	/*handel alt+i=insert:
	when scancode is 0xD1 and not shift+R/L is pressed,report insert keyevents;
	when event is press, mark virtual_INS = true;
	*/
	if(ev->scancode == 0xD1){
		if(ev->state == KEY_STATE_PRESSED){
			if((!ctx->phy_SHL)&&(!ctx->phy_SHR)){// Alt + I = Insert
				input_report_key(ctx->input_dev, KEY_LEFTALT, 0);
				input_report_key(ctx->input_dev, KEY_INSERT, 1);
				ctx->virtual_INS = true;			
			}else
			if((!ctx->phy_SHL)&&(ctx->phy_SHR)){// SHR + Enter = Mosemode Switch
				ctx->mouse_mode = !(ctx->mouse_mode);
			}else
			if((ctx->phy_SHL)&&(!ctx->phy_SHR)){// SHL + Enter = Shift + Enter
				input_report_key(ctx->input_dev, KEY_ENTER, 1);
			}
			// SHL + SHR + Enter = None
		}else
		if(ev->state == KEY_STATE_RELEASED){
			if((!ctx->phy_SHL)&&(!ctx->phy_SHR)){
			input_report_key(ctx->input_dev, KEY_INSERT, 0);
			ctx->virtual_INS = false;
			}else
			if((ctx->phy_SHL)&&(!ctx->phy_SHR)){
	...
	// Handle fn key mappings
	bool handle_fn_key = proccess_fn_key(ctx, ev);
	if (handle_fn_key) {
	...
	// when keycode = KEY_I and virtual_INS is true,send a insert key release event
	if(keycode == KEY_I && ctx->virtual_INS){
		input_report_key(ctx->input_dev, KEY_INSERT, 0);
		ctx->virtual_INS = false;
		return;
	}
	...
	// Report normal keys to input system
...
static void input_workqueue_handler(struct work_struct *work_struct_ptr)
{
	...
	// Handle mouse movement
		...
		else if (press_time <= 2 * mouse_fast_move_thr_time)
		{
			mouse_move_step = 1;
		}
		else if (press_time <= 6 * mouse_fast_move_thr_time)
		{
			mouse_move_step = 2;
		}
		else
...
int input_probe(struct i2c_client* i2c_client)
{
    ...
    */
	...
	//input_set_capability(g_ctx->input_dev, EV_REL, REL_WHEEL);
...
*/
	...
	// Initialize mouse mode flags
    g_ctx->mouse_mode = false;
    g_ctx->mouse_move_dir = 0;
	// Initialize key status flags
	g_ctx->phy_SHR = false;
	g_ctx->phy_SHL = false;
	g_ctx->virtual_INS = false;

	/* initialize mouse throttle: default send every 2 polls (128Hz/2 = 64Hz) */
    // g_ctx->mouse_tick = 0;
    // g_ctx->mouse_report_div = 2;
...
static ssize_t battery_percent_show(struct kobject *kobj, struct kobj_attribute *attr,char *buf)
{
	int percent;
...
struct kobj_attribute battery_percent_attr= __ATTR(battery_percent, 0444, battery_percent_show, NULL);

// Keyboard backlight value
static ssize_t __used keyboard_backlight_store(struct kobject *kobj,struct kobj_attribute *attr, char const *buf, size_t count)
{
	return parse_and_write_i2c_u8(buf, count, REG_ID_BK2);
}
struct kobj_attribute keyboard_backlight_attr= __ATTR(keyboard_backlight, 0220, NULL, keyboard_backlight_store);

// screen backlight value
static ssize_t __used screen_backlight_store(struct kobject *kobj,struct kobj_attribute *attr, char const *buf, size_t count)
{
	return parse_and_write_i2c_u8(buf, count, REG_ID_BKL);
}
struct kobj_attribute screen_backlight_attr= __ATTR(screen_backlight, 0220, NULL, screen_backlight_store);

// Time since last keypress in milliseconds
static ssize_t last_keypress_show(struct kobject *kobj, struct kobj_attribute *attr,char *buf)
{
...
struct kobj_attribute last_keypress_attr= __ATTR(last_keypress, 0444, last_keypress_show, NULL);
...
MODULE_DESCRIPTION("keyboard driver for picocalc,improve by beapig");
MODULE_VERSION("0.02");